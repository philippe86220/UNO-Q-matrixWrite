...
Dans la fonction `loop()`, on utilise généralement `repeat = 1`, puisque `loop()` est elle-même appelée en permanence  
par le framework Arduino. Dans d’autres cas, par exemple lorsqu’on attend un signal en provenance du cœur Linux (comme dans 
